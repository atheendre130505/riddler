...
async def process_url(url: str = Form(...), provider: str = Form("gemini")):
    ...
    try:
        ...
        import asyncio
        recap = await analyzer.enhanced_generate_recap(data["content"], "medium")
        questions = await analyzer.enhanced_create_questions(data["content"], 5)
        ...
        return {
            ...
            "recap": recap,
            "quiz": questions,
            "key_concepts": await analyzer.enhanced_extract_key_concepts(data["content"])
        }
...
async def health_check():
    ...
    test_analyzer = EnhancedContentAnalyzer(provider="gemini")
    ...
    return {"status": "healthy", "components": {
        ...
        "rag": "ready" if hasattr(test_analyzer, 'advanced_rag') else "fallback"
    }}
...
async def rag_stats():
    ...
    test_analyzer = EnhancedContentAnalyzer(provider="gemini")
    if hasattr(test_analyzer, 'advanced_rag'):
        return test_analyzer.advanced_rag.get_document_stats()
    else:
